...
const TrainerList = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetAllUsersQuery({ page, fullName, role: "Trainer" });
    ...
    <section>
      ...
      <div className="flex justify-between  items-center">
        ...
        <p className="text-sm text-gray-400">
          Total Trainers : {trainers?.totalUsers}
        </p>
      </div>
      ...
      <div className="flex justify-center gap-4 mt-4">
        ...
        <button
          disabled={page === trainers?.totalPages}
          onClick={() => setPage(page + 1)}
          className={`bg-[var(--grayLight)] px-3 py-0.5 rounded font-semibold cursor-pointer transition-opacity duration-300 ${
            !trainers?.nextPage ? "opacity-30 cursor-not-allowed" : ""
          }`}
        >
          Next
        </button>
